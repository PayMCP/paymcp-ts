...
export class WalleotProvider extends BasePaymentProvider {
  ...
  async createPayment(
    ...
    this.logger.debug(`[WalleotProvider] createPayment ${amount} ${currency} "${description}"`);
    const payload = {
      amount: Math.round(amount * 100), // amount in cents
      currency: currency.toLowerCase(),
      description,
    };
    const session = await this.request<any>('POST', `${BASE_URL}/sessions`, payload);
    // API expected: { sessionId, url, ... }
    if (!session?.sessionId || !session?.url) {
      throw new Error('[WalleotProvider] Invalid response from /sessions (missing sessionId/url)');
    }
  ...
  async getPaymentStatus(paymentId: string): Promise<string> {
    ...
    const session = await this.request<any>('GET', `${BASE_URL}/sessions/${paymentId}`);
    // API expected: { status }
