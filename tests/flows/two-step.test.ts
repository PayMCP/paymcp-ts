...
describe("TwoStepFlow", () => {
  ...
  describe("Payment Initiation (Step 1)", () => {
    ...
    it("should handle payment creation errors", async () => {
      ...
      vi.spyOn(mockProvider, "createPayment").mockRejectedValue(
      ...
      await expect(wrapper({ input: "test" }, {})).rejects.toThrow(
        "Payment API error",
      );
    });
  });
  ...
  describe("Payment Confirmation (Step 2)", () => {
    it("should confirm payment and execute original tool", async () => {
      ...
      expect(mockProvider.getPaymentStatus).toHaveBeenCalledWith(
        "mock_payment_123",
      );
    ...
    it("should handle unpaid payment status", async () => {
      ...
      vi.spyOn(mockProvider, "getPaymentStatus").mockImplementationOnce(
        async () => "pending",
      );
    ...
    it("should handle payment status check errors", async () => {
      ...
      vi.spyOn(mockProvider, "getPaymentStatus").mockRejectedValueOnce(
        new Error("API Error"),
      );
