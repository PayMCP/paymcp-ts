...
describe("StripeProvider", () => {
  ...
  describe("createPayment", () => {
    ...
    it("should handle API errors", async () => {
      ...
      await expect(
        provider.createPayment(10.0, "USD", "Test payment"),
    ...
    it("should convert amount to cents correctly", async () => {
      ...
      expect(body.toString()).toContain(
        "line_items%5B0%5D%5Bprice_data%5D%5Bunit_amount%5D=1099",
      );
    });
  });
  ...
  describe("getPaymentStatus", () => {
    ...
    it("should handle API errors", async () => {
      ...
      await expect(provider.getPaymentStatus("cs_test_123")).rejects.toThrow(
        "HTTP 404",
      );
    });
  });
