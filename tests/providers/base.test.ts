...
describe('BasePaymentProvider', () => {
  ...
  describe('request', () => {
    it('should make GET request with query params', async () => {
      ...
      const result = await provider.testRequest('GET', 'https://api.test.com/data', {
        foo: 'bar',
        baz: 123,
      });
    ...
    it('should handle GET request with empty data', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('https://api.test.com/data', expect.any(Object));
    });
    ...
    it('should handle GET request with URL that already has query params', async () => {
      ...
      await provider.testRequest('GET', 'https://api.test.com/data?existing=param', {
        new: 'param',
      });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(provider.testRequest('GET', 'https://api.test.com/data')).rejects.toThrow(
        'Network failed'
      );
    ...
    it('should handle HTTP errors', async () => {
      ...
      await expect(provider.testRequest('GET', 'https://api.test.com/data')).rejects.toThrow(
        'HTTP 404 https://api.test.com/data'
      );
