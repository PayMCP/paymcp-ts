...
describe("ElicitationFlow", () => {
  ...
  it("should handle rejected payment", async () => {
    ...
    (mockProvider.getPaymentStatus as vi.Mock).mockResolvedValueOnce(
      "canceled",
    );
    ...
    expect(result.content).toEqual([
      { type: "text", text: "Payment canceled by user." },
    ]);
  ...
  it("should handle payment creation failure", async () => {
    ...
    (mockProvider.createPayment as vi.Mock).mockRejectedValueOnce(
    ...
    await expect(wrapper({ test: "data" }, extra)).rejects.toThrow(
      "Payment API error",
    );
