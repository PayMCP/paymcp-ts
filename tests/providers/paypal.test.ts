...
import { PayPalProvider, createPayPalProvider } from '../../src/providers/paypal';
...
describe('PayPalProvider', () => {
  ...
  describe('_getToken', () => {
    ...
    it('should handle token request errors', async () => {
      ...
      await expect((provider as any).getAccessToken()).rejects.toThrow('Failed to get access token: 401');
    });

    it('should handle network errors', async () => {
      fetchSpy.mockRejectedValueOnce(new Error('Network error'));

      await expect((provider as any).getAccessToken()).rejects.toThrow('Network error');
    });
  });
    ...
    it('should handle missing approve link', async () => {
      fetchSpy.mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          status: 'CREATED',
          links: [
            { href: 'https://other.link', rel: 'other', method: 'GET' },
          ],
        }),
      } as Response);
  ...
  describe('getPaymentStatus', () => {
    ...
    it('should handle APPROVED status with auto-capture', async () => {
      ...
      expect(captureCall[0]).toBe('https://api-m.sandbox.paypal.com/v2/checkout/orders/ORDER123/capture');
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        provider.getPaymentStatus('ORDER123')
