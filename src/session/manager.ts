...
export class SessionManager {
  ...
  static createStorage(config?: SessionStorageConfig): ISessionStorage {
    ...
    switch (storageConfig.type) {
      ...
      case 'redis':
        throw new Error('Redis storage not yet implemented. Use memory storage for now.');

      case 'custom':
        if (storageConfig.options?.implementation) {
          return storageConfig.options.implementation as ISessionStorage;
        }
        throw new Error('Custom storage requires an implementation in options.implementation');
